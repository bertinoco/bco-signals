...
jd-source/             — Verbatim JD text archive, one file per entry (and per
                          rejected posting), keyed to jobs.json by id
  README.md             — File format, front matter fields, exclusion rules

jd-insights/            — What the dataset adds up to; not published to docs/
  stats.md              — Generated by refresh.py — never hand-edit
  quotes.md             — Generated by refresh.py — never hand-edit
  patterns.md           — Hand-written notes on how postings are written
  findings.md           — Hand-written, the only editorialized file in the repo
  refresh.py            — Regenerates stats.md and quotes.md from jobs.json + jd-source/
